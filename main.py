...
# @dp.message_handler(content_types=['text']) Работает, но если запустить, то не будет работать остальное
# async def soob(message: types.Message):
#      if message.from_user.id != admin_ip:
#         msg = 'ID пользователя: ' + str(message.from_user.id) + ', Сообщение: ' + str(message.text)
